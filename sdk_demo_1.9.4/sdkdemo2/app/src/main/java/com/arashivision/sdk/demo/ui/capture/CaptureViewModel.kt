...
class CaptureViewModel : BaseViewModel(), IPreviewStatusListener, ICaptureStatusListener {
    ...
    private fun initCapture() {
        ...
        viewModelScope.launch {
            ...
            emitEvent(CaptureEvent.InitCaptureEvent(EventStatus.SUCCESS, captureModeList = instaCameraManager.supportCaptureMode, currentCaptureMode = cameraOfflineData.currentCaptureMode, isSingleClickAction = isSingleClickAction()))
        }
    }
    ...
    fun startCapture() {
        ...
        when {
            ...
            !currentCaptureMode.isLiveMode && !instaCameraManager.isCameraWorking -> startRecord(currentCaptureMode)
    ...
    override fun onCaptureStarting() {
        emitEvent(CaptureEvent.CameraCaptureEvent(captureMode = cameraOfflineData.currentCaptureMode, isSingleClickAction = isSingleClickAction(), CaptureEvent.CaptureStatus.STARTING))
    }

    override fun onCaptureWorking() {
        emitEvent(CaptureEvent.CameraCaptureEvent(captureMode = cameraOfflineData.currentCaptureMode, isSingleClickAction = isSingleClickAction(), CaptureEvent.CaptureStatus.WORKING))
    }

    override fun onCaptureStopping() {
        emitEvent(CaptureEvent.CameraCaptureEvent(captureMode = cameraOfflineData.currentCaptureMode, isSingleClickAction = isSingleClickAction(), CaptureEvent.CaptureStatus.STOPPING))
    }

    override fun onCaptureFinish(paths: Array<String>?) {
        emitEvent(CaptureEvent.CameraCaptureEvent(captureMode = cameraOfflineData.currentCaptureMode, isSingleClickAction = isSingleClickAction(),CaptureEvent.CaptureStatus.FINISH))
        onCaptureFinishEnd()
    ...
    override fun onCaptureTimeChanged(captureTime: Long) {
        ...
        if (timeLapseWorking) {
            ...
            emitEvent(CaptureEvent.CameraCaptureEvent(captureMode = cameraOfflineData.currentCaptureMode, isSingleClickAction = isSingleClickAction(), CaptureEvent.CaptureStatus.RECORD_TIME, captureTime, videoTime))
        } else {
            emitEvent(CaptureEvent.CameraCaptureEvent(captureMode = cameraOfflineData.currentCaptureMode, isSingleClickAction = isSingleClickAction(), CaptureEvent.CaptureStatus.RECORD_TIME, recordTime = captureTime))
        }
    }

    override fun onCaptureCountChanged(captureCount: Int) {
        emitEvent(CaptureEvent.CameraCaptureEvent(captureMode = cameraOfflineData.currentCaptureMode, isSingleClickAction = isSingleClickAction(), CaptureEvent.CaptureStatus.CAPTURE_COUNT, captureCount = captureCount))
    }
    ...
    fun cameraPreviewStreamParamsChanged(playerView: InstaCapturePlayerView) {
        ...
        if (playerView.isPlaying) {
            ...
            val windowCropInfo = if (shouldUpdateWindowCrop && assetInfo != null) { createWindowCropInfo(assetInfo!!) } else null
